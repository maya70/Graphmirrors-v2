(function($P){
	...
	$P.D3TreeRing = $P.defineClass(
		...
		function D3TreeRing(config) {
			...
			this.nodesJSON = config.dataTree || {}; 
			this.gallusOrth = [];
			this.badPathwayIDs = []; 
			this.badPathwayNames = [];
			this.found = false; 
			this.newNode = null; 
			this.color = {
			...
			return this;},
		{
			...
			searchTree: function(n, name) {
						...
						n.children.forEach(function(node) {
							if(!self.found)  
							 	 self.searchTree(node, name);
							
						});
						
			},
			bfs: function(n, index)
								{
									var self = this; 
									self.matchOrtholog(n,index++);						
									if (!n.children) {  // leaf node
										return;
									}
									n.children.forEach(function(n) {self.bfs(n, index); });
								},

			matchOrtholog: function(pathway, index) {
				var self = this; 
				pathway.symbols = [];
				pathway.gallusOrth = {};
				pathway.gallusOrth.sharedSymbols = [];
				if(index > 0)
					{							
						console.log('pathways['+ index +'].dbId =' + pathway.dbId);
						$P.getJSON('./php/get_entities.php',
												function (jsonData) {
													if (jsonData.entities) {
														$.each(jsonData.entities, function(entityId, entity) {
															//if (!entity.component_of) {  // not a subsumed entity
																	pathway.symbols.push(entity.name);
												...
												function() {
													self.badPathwayIDs.push(pathway.dbid);
													self.badPathwayNames.push(pathway.name);
												});
										var count =0; 
										for(var i = 0; i < pathway.symbols.length; i++) {
											for(var k = 0; k < self.gallusOrth.length; k++ ) {
												if (self.gallusOrth[k].dataBaseId !== "\N") {
													if(pathway.symbols[i] === self.gallusOrth[k].symbol) {
														count++;
														pathway.gallusOrth.sharedSymbols.push(pathway.symbols[i]);
														break;
													}
												}
											}
										...
										pathway.gallusOrth.count = count;
										 if (count === pathway.symbols.length) {
					                            pathway.gallusOrth.type = "Complete";
					                        }
				                        else if (count === 0&&0 !== pathway.symbols.length) {
				                            pathway.gallusOrth.type = "Empty";

				                        }
				                        else {
				                            pathway.gallusOrth.type = "Part";
				                        }                  
			...
			updateJSON: function(){
				...
				function printInfo(){
                                var minX = minY = minDx = minDy = 100000000;
                                var maxX = maxY = maxDx = maxDy = 0;
                                for(var i = 0; i < nodeData.length; i++)
                                {
                                    if(nodeData[i].x < minX) minX = nodeData[i].x;
                                    if(nodeData[i].y < minY) minY = nodeData[i].y;
                                    if(nodeData[i].dx < minDx) minDx = nodeData[i].dx;
                                    if(nodeData[i].dy < minDy) minDy = nodeData[i].dy;
 
                                    if(nodeData[i].x > maxX) maxX = nodeData[i].x;
                                    if(nodeData[i].y > maxY) maxY = nodeData[i].y;
                                    if(nodeData[i].dx > maxDx) maxDx = nodeData[i].dx;
                                    if(nodeData[i].dy > maxDy) maxDy = nodeData[i].dy;
                                }
                                console.log('min x = '+ minX);
                                console.log('min y = '+ minY);
                                console.log('min dx = '+ minDx);
				...
				function queryByIds(type) {
								url = "http://reactome.org/ReactomeRESTfulAPI/RESTfulWS/queryById/DatabaseObject/R-ALL-29358.1";
                    ...
                    if (xml.children.length) {
                        ...
                        childTypes.forEach(function(type) {
                            ...
                            if (type.values.length == 1) {
                                ...
                                if(type.key === 'Pathway') type.key = 'children';
                                if(xml.attributes)
                                  {
                                    dbid = (xml.attributes['dbId'])? xml.attributes['dbId'].value : '0';
                                    //console.log(dbid);
                                    name = (xml.attributes['displayName'])? xml.attributes['displayName'].value : 'homo sapiens';
                                    //console.log(name);
                                  }
 
                                //type.values[0] = $.extend(type.values[0], dbid);
                                //type.values[0] = $.extend(type.values[0], name);
                               o['dbId'] = dbid;
                               o['name'] = name;
                               ...
                               o[type.key] = type.values.map( function(v) {
                                ...
                                o['name'] = name;
                                o[type.key] = type.values.map( function(v) {
                                        //v = $.extend(v, {'dbId': dbid.value? dbid.value: '0'} );
                                        //v = $.extend(v, {'name': name.value? name.value: 'homo sapiens'});
                                        return xmlToJSON(v);
                                    });
                ...
                //TODO: update the file homo.xml by fetching from RESTful here
			    ...
			    $.ajax({
				                    type: "GET",
				                    url: "./data/Ortholog/gallus_gallus_ortholog.txt",
				                    dataType: "text",
				                    ...
				                    success: function (txt) {
				                        if(txt.length)
				                        {
				                            ...
				                            for(var i=1; i<lines.length; ++i)
				                            {
				                                ...
				                                var obj = {};
				                                obj.symbol = arrays[0];
				                                obj.dataBaseId = arrays[1];
				                                gallusOrth.push(obj);
				                            }
				                            console.log(gallusOrth[0]);
				                            //var file = self.matchFile(objects, root,gallusOrth);
				                        }
				                    }
				                }); */

				$P.getTXT('./php/load_ortholog.php',
							function (txt) {
				                        if(txt.length)
				                        {   
				                            var lines = txt.split("\r\n");
				                            for(var i=1; i<lines.length; ++i)
				                            {
				                                ...
				                                obj.symbol = arrays[0];
				                                obj.dataBaseId = arrays[1];
				                                self.gallusOrth.push(obj);
				                            }
				                            console.log(self.gallusOrth[0]);
				                            //var file = self.matchFile(objects, root,gallusOrth);
				                        }
				                    },
				                    {type: 'GET'}
				...
				console.log('now reading pathway hierarchy..');
				/*
                $P.getXML('./data/homo.xml', function(data){
                                //console.log(data.childNodes[0].childNodes);
                                var node, count, minRatio, maxRatio, progress;
                                //self.nodesJSON = xmlToJSON(data.childNodes[0]);
                                self.nodesJSON = {};
                                self.nodesJSON = self.parseToJSON(data);
								
                                //nodeData = partition.nodes(self.nodesJSON);
                                /*
                                var nameO = {'name': 'homo sapiens'};
                                if(!nodeData[0].name)
                                    nodeData[0] = $.extend(nodeData[0], nameO);
                                ...
                                var count = 0;
                                nodeData.forEach(function(node, nodeId){
                                    count++;
                                    if(node === '')
                                      {
                                        nodeData[nodeId] = nodeData[nodeId - 1];  
                                      }

                                      
                                });
*/
                                // write the JSON hierarchy to file
                                /*
                                $P.getJSON('./php/write_hierarchy.php',
												function (jsonData) {},
												{type: 'POST', 
												 data: {
														node: JSON.stringify(self.nodesJSON)
														}
												},
												function() {
								...
								*/
								...
								/*
								// Read symbols from RESTful api
								$.ajax({
				                    type: "GET",
				                    url: "http://reactome.org/ReactomeRESTfulAPI/RESTfulWS/pathwayComplexes/109606",
				                    dataType: "json",
				                    success: function (jsonData) {
				                        if(jsonData.length)
				                        {
				                           ...
				                           console.log('Number of complexes = '+ numComplexes);
				                        }
				                    }
				                });
								*/

					$P.getJSON('./php/read_hierarchy.php', function(nodesJSON){
								// Read symbols from Sqlite 3
								self.nodesJSON = nodesJSON; 
								self.bfs(self.nodesJSON, 0);
								console.log('Number of bad pathways: '+ self.badPathwayIDs.length);
						...
						{   //main
							...
							if(_this.file){
								$P.getJSON(_this.file, function (root, error) {
									...
									//nodeData.forEach(function(n){	self.bfs(n); });
								...
								var node, count, minRatio, maxRatio, progress;
								self.searchTree(self.nodesJSON, self.name);
								var root = self.newNode; 
								nodeData = partition.nodes(root);
									...
									if (classes && classes.length) {
										...
										classes.forEach(function(klass) {
											...
											for (var ijk = 0; ijk < targets.length; ++ijk) {
											    ...
											    if(importObj.source && importObj.target)	links.push(importObj);
											}});
										link = link
											.data(bundle(links))
											.enter().append('path')
									...
									function click(d, i) {
										...
										$P.state.scene.addLink(
											new $P.BubbleLink({
												source: new $P.D3TreeRing.BubbleLinkEnd({
													ring: self.parent,
													datum: selectedData //d3.select(this).datum()
												}),
												target: new $P.BubbleLink.End({object: ringBubble})
											}));
